...
export default function WordCounter() {
  ...
  // Memoize calculations to prevent unnecessary re-renders on large text
  const stats = useMemo(() => {
    ...
    return {
      ...
      readingTime: Math.ceil((trimmedText ? trimmedText.split(/\s+/).length : 0) / 200) // Avg 200 wpm
    };
  }, [text]);
        ...
        {text && (
          <button 
            ...
            className="absolute top-6 right-6 px-4 py-2 bg-white/80 backdrop-blur-md border border-slate-200 
                       rounded-xl text-xs font-bold text-slate-500 hover:text-red-500 transition-colors shadow-sm"
          >
            CLEAR TEXT
          </button>
        )}
...
// Reusable Stat Component to keep the DOM clean
