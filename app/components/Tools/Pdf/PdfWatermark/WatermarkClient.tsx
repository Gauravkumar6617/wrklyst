  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (watermarkImg) {
        ...
        const isPng =
          watermarkImg.type === "image/png" ||
          watermarkImg.name.toLowerCase().endsWith(".png");
      ...
      const targetIndices =
        pageScope === "single" ? [viewPage - 1] : pages.map((_, i) => i);
      ...
      const blob = new Blob([Buffer.from(finalPdfBytes)], {
      ...
      setTimeout(() => {
        router.push(
          `/download/${fileId}?name=watermarked_${pdfFile.name}&tool=Watermark PDF&local=true`,
        );
      }, 1500);
    } catch (error: any) {
            ...
            <div className="space-y-4 pt-6 border-t border-slate-50">
              ...
              <div className="flex bg-slate-100 p-1 rounded-2xl">
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${!watermarkImg ? "bg-white shadow-sm text-blue-600" : "text-slate-500"}`}
                >
                  TEXT
                </button>
                <label
                  className={`flex-1 py-2 rounded-xl text-xs font-bold text-center cursor-pointer transition-all ${watermarkImg ? "bg-white shadow-sm text-blue-600" : "text-slate-500"}`}
                >
                  IMAGE
                  <input
                    type="file"
                    ...
                    className="hidden"
                    onChange={(e) =>
                      setWatermarkImg(e.target.files?.[0] || null)
                    }
                  />
                </label>
              </div>
              ...
              {watermarkImg ? (
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-4 relative">
                  ...
                  <button
                    onClick={() => setWatermarkImg(null)}
                    className="p-1.5 bg-white rounded-full shadow-sm text-red-500 hover:scale-110 transition-transform"
                  >
                    <X size={14} />
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="bg-slate-50 w-full p-3 rounded-xl text-xs font-bold focus:ring-1 ring-blue-500"
                  />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-300 font-bold ml-1 uppercase">
            ...
            {!pdfFile ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-10 text-center cursor-pointer">
                ...
                <p className="text-slate-400 text-sm max-w-sm mx-auto leading-relaxed font-medium">
                  Click or drag a PDF file to enable the real-time visual
                  watermarking engine.
                </p>
              </div>
            ) : (
              <div className="relative w-full h-full bg-white overflow-hidden">
                ...
                <div className="absolute top-8 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 bg-white/90 backdrop-blur-xl px-6 py-3 rounded-3xl border border-slate-200 shadow-2xl">
                  ...
                  <div className="text-[10px] font-black text-slate-600">
                    {pageScope === "all" ? "BATCH MODE" : "SINGLE MODE"}
                  </div>
                </div>
              </div>
            )}
