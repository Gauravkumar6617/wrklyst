        ...
        <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 md:px-8 py-4 bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.05)] rounded-[24px] relative">
                ...
                <div className="bg-white shadow-[0_40px_80px_rgba(0,0,0,0.1)] rounded-[32px] border border-slate-100 overflow-hidden flex flex-col min-h-[550px]">
                  ...
                  <div className="flex flex-1">
                    ...
                    <div className="w-1/4 bg-slate-50/50 p-8 border-r border-slate-100">
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-600 mb-6 px-4">
                        Categories
                      </p>
                      ...
                      <div className="grid grid-cols-3 gap-4 overflow-y-auto max-h-[350px] pr-4 custom-scrollbar">
                        {activeCategory &&
                          groupedTools[activeCategory]?.map((tool) => (
                            ...
                            >
                              <p className="font-black text-[#2D2E5F] truncate">
                                {tool.name}
                              </p>
                              <p className="text-[11px] text-slate-700 font-medium mt-1 line-clamp-1">
                                {tool.description}
                              </p>
                            </Link>
                          ))}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm font-semibold text-slate-600">
                        ...
                        <span className="text-slate-600 font-normal">
                          We build custom tools based on user requests.
                        </span>
                      </p>
                    </div>
                    <Link
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {!isLoggedIn ? (
              ...
              <div className="flex items-center gap-2 bg-[#5D5FEF]/5 px-3 py-2 sm:px-4 sm:py-2 rounded-full border border-[#5D5FEF]/10">
                <span className="text-xs sm:text-sm font-black text-[#2D2E5F] truncate">
                  {username}
                </span>
                <button
                  onClick={logout}
                  className="text-red-500 hover:text-red-700 hover:bg-red-50 p-1.5 rounded-lg transition-all duration-200 active:scale-95"
                  title="Logout"
                >
                  <LogOut size={18} />
                </button>
              ...
              >
                <div className="bg-white/95 backdrop-blur-2xl border border-white/60 shadow-[0_20px_50px_rgba(0,0,0,0.15)] rounded-[32px] p-4 flex flex-col gap-2 overflow-hidden">
                  <Link
                    href="/about"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-base font-bold p-4 hover:bg-[#5D5FEF]/5 rounded-2xl text-[#2D2E5F]"
                  >
                    About
                  </Link>
                  <Link
                    href="/tools"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-base font-bold p-4 hover:bg-[#5D5FEF]/5 rounded-2xl text-[#2D2E5F] flex justify-between items-center"
                  >
                    Explore Tools{" "}
                  ...
                  <div className="mx-2 mt-2 p-5 bg-slate-50 border border-slate-100 rounded-3xl space-y-3">
                    <p className="text-[11px] font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2">
                      <Info size={14} className="text-[#5D5FEF]" /> Tool not
                      present?
                    </p>
                    <p className="text-sm font-semibold text-[#2D2E5F]">
                      We surely add that! Message us with your requirements.
                    </p>
                    <Link
                      href="/contact"
                      onClick={() => setIsMobileMenuOpen(false)}
